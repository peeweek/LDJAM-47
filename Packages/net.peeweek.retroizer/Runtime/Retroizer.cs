...
namespace Retroizer
{
    ...
    public class Retroizer : MonoBehaviour
    {
        ...
        const string kQualityLow = "QUALITY_LOW";
        const string kQualityMid = "QUALITY_MID";
        const string kQualityHigh = "QUALITY_HIGH";
        ...
        {
            m_Material.SetVector("OverscanMaskHardness", OverscanMaskHardness);

            m_Material.DisableKeyword(kQualityLow);
            m_Material.DisableKeyword(kQualityMid);
            m_Material.DisableKeyword(kQualityHigh);

            string qualitySF = string.Empty;
            switch (m_Quality)
            {
                default:
                case Quality.Low:
                    qualitySF = kQualityLow;
                    break;
                case Quality.Medium:
                    qualitySF = kQualityMid;
                    break;
                case Quality.High:
                    qualitySF = kQualityHigh;
                    break;
            }

            m_Material.EnableKeyword(qualitySF);

        }

        [SerializeField]
        Vector2 OverscanMaskHardness = new Vector2(12, 12);
